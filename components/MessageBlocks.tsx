...
export const ToolResultBlock: React.FC<{ content: string, toolName?: string }> = ({ content, toolName }) => {
  ...
  let isUrl = false;
  let isImage = false; // Could add specific handling for image URLs later if needed

  try {
    const parsed = JSON.parse(content);
    // If it's a simple JSON object for status or empty data, might still be considered "empty"
    if (Object.keys(parsed).length === 0 || (parsed.status && Object.keys(parsed).length === 1)) {
        // Don't pretty print purely empty/status JSON, treat as effectively empty for rendering decision
        displayedContent = ''; 
    } else {
        displayedContent = JSON.stringify(parsed, null, 2); // Pretty print JSON
    }
  } catch (e) {
    ...
    try {
        ...
        if (url.protocol === 'http:' || url.protocol === 'https:') {
            isUrl = true;
            // Optionally check if it's an image URL by extension
  ...
  // If content is still effectively empty after processing, don't render the block
  // unless there's a toolName which provides some context.
  if (!displayedContent.trim() && !toolName) {
      return null;
  }
  ...
  return (
    <div className="my-2 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden bg-gray-50 dark:bg-gray-800/50">
      ...
      >
        ...
        <span>Tool Result {toolName ? `: ${toolName}` : ''}</span>
        {isExpanded ? <ChevronDown className="w-3 h-3 ml-auto" /> : <ChevronRight className="w-3 h-3 ml-auto" />}
      </button>
        ...
        <div className="px-3 py-2 text-sm text-gray-700 dark:text-gray-300 font-mono border-t border-gray-200 dark:border-gray-700 bg-white/50 dark:bg-black/20">
          {isImage ? (
            <a href={content} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline block mb-2">
              <img src={content} alt={`Generated by ${toolName || 'tool'}`} className="max-w-full h-auto rounded-md" />
            </a>
          ) : isUrl ? (
            ...
            <MarkdownRenderer content={displayedContent || "No detailed output."} />
          )}
        </div>
      )}
    </div>
